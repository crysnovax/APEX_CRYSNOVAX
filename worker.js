...
export default {
  async fetch(request, env, ctx) {
    ...
    function isAuthenticated(request) {
      ...
      if (!expectedToken) return true; // No permanent token set → allow all
    ...
    async function generateTempToken(githubId) {
      ...
      if (existing) {
        ...
        if (data.expires > Date.now()) {
          return data.token; // Return existing valid token
        }
        return null; // Expired, cannot generate a new one
      }
    ...
    // ==================== IMAGE UPLOAD HELPER (for vision) ====================
    ...
    try {
      ...
      if (path === '/changebg' && method === 'POST') {
        ...
        if (!imageFile || !prompt) return new Response(JSON.stringify({ error: 'Missing image or prompt' }), { status: 400, headers: corsHeaders });
        ...
        if (!apiRes.ok) return new Response(JSON.stringify({ error: `Upstream error: ${apiRes.status}` }), { status: apiRes.status, headers: corsHeaders });
...
<head>
  ...
  <title>APEX🜲 CRYSN⚉VA AI · Gateway</title>
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .endpoints-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:20px;margin:2rem 0}
    .card{background:rgba(20,15,10,0.6);backdrop-filter:blur(8px);border:1px solid rgba(212,175,55,0.2);border-radius:24px;padding:1.5rem;transition:all 0.3s;box-shadow:0 10px 20px rgba(0,0,0,0.4)}
    ...
    .endpoint-path{font-family:monospace;font-size:1rem;color:#d4af37}
...
<body>
  ...
  <div class="container">
    ...
    <div class="token-actions">
      ...
      <a href="https://wa.me/message/636PEVHM5BZUM1" target="_blank" style="text-decoration:none"><button>𓊈𝑽꯭𝑰꯭𝑷ࠡࠡࠡࠡࠢ𓊉 Purchase Permanent Token</button></a>
    </div>
  ...
  <script>
    const endpoints = ${JSON.stringify(Object.entries(proxyMap).map(([p, d]) => ({ method: 'GET/POST', path: p, desc: d })))};
    let currentToken = '';
    ...
    function renderCards(){
      ...
      endpoints.forEach(ep => {
        ...
        else if (ep.path.includes('rembg')) requiresKey = 'removeBg';
        ...
        card.className = 'card';
        card.innerHTML = '<div class="card-header"><span class="method">' + ep.method + '</span><span class="endpoint-path">' + ep.path + '</span></div><p>' + ep.desc + '</p><div class="status-indicator">' + statusDot + '</div><button class="copy-btn" data-path="' + ep.path + '">📋 Copy URL</button>';
    ...
    applyBtn.onclick = () => { currentToken = tokenInput.value.trim(); renderCards(); };
