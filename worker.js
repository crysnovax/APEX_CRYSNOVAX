...
// AI Chat: OpenRouter (free models) · Image Gen: Prodia (free tier)
export default {
    ...
    if (path === '/' && method === 'GET') {
        const endpointsByCategory = [
            { category: '🤖 AI Chat PRO 🜲', endpoints: [
                { method: 'POST', path: '/chat', desc: 'Unified AI Chat (DeepSeek, GPT, Gemini, Llama, etc.)' }
            ] },
            { category: '🎨 Image Generation ✐', endpoints: [
                { method: 'POST', path: '/generate-image', desc: 'AI Image Generation (Anime, Realistic, Cartoon, Oil, Watercolor, etc.)' },
...
<body>
  ...
  <script>
    ...
    function renderCategories() {
      ...
      categories.forEach(cat => {
        ...
        cat.endpoints.forEach(ep => {
          ...
          else if (ep.path === '/chat') requiresKey = 'openRouter';
          else if (ep.path === '/generate-image') requiresKey = 'prodia';
    ...
    // ==================== API ROUTES ====================
    try {
        // ----- EXISTING AI SERVICES (Groq, OCR, etc.) -----
        ...
        // ==================== OPENROUTER UNIFIED AI CHAT (FREE MODELS ONLY) ====================
        if (path === '/chat' && method === 'POST') {
            ...
            // Whitelist of known free model IDs on OpenRouter
            const FREE_MODEL_IDS = new Set([
                ...
                'stealth/hunter-alpha:free',
                'stealth/healer-alpha:free',
                'arcee-ai/trinity-large-preview:free',
                'sophosympatheia/rogue-rose-103b:free',
                'lynn/soliloquy-l3:free',
                'featherless/qwerky-72b:free'
            ]);

            // Use the client-provided model if it's in the whitelist; otherwise fallback to openrouter/free
            let selectedModel = (model && FREE_MODEL_IDS.has(model)) ? model : 'openrouter/free';
            ...
            try {
                const apiRes = await fetch(openRouterUrl, {
                    ...
                    headers: {
                        ...
                        'Content-Type': 'application/json',
                        'HTTP-Referer': 'https://api.crysnovax.link',
                        'X-OpenRouter-Title': 'CRYSNOVA AI Gateway'
                    },
                    body: JSON.stringify({
                        model: selectedModel,
                ...
                return new Response(JSON.stringify({ response: reply, model_used: selectedModel }), {
                    headers: { ...corsHeaders, 'Content-Type': 'application/json' }
                });
            } catch (err) {
        ...
        // Redirect legacy endpoints to unified /chat (using free models)
        const LEGACY_CHAT_REDIRECTS = {
            '/deepseek': 'deepseek/deepseek-chat:free',
            '/gemini': 'google/gemini-2.0-flash-exp:free',
            '/ai/chateverywhere': 'deepseek/deepseek-chat:free',
            '/ai/copilot': 'microsoft/phi-4-mini-instruct:free',
            '/ai/ai4chat': 'deepseek/deepseek-chat:free',
        };

        for (const [legacyPath, modelCode] of Object.entries(LEGACY_CHAT_REDIRECTS)) {
            if (path === legacyPath && method === 'POST') {
                const body = await request.json();
                const prompt = body.query || body.text || body.prompt || '';
                const newRequest = new Request(url.origin + '/chat', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ prompt, model: modelCode })
                });
                return fetch(newRequest);
            }
        }

        // ==================== PRODIA UNIFIED IMAGE GENERATION ====================
        if (path === '/generate-image' && method === 'POST') {
            ...
            if (style === 'anime') enhancedPrompt = `${prompt}, anime style, studio ghibli`;
            else if (style === 'realistic') enhancedPrompt = `${prompt}, photorealistic, 8k, detailed`;
            else if (style === 'cartoon') enhancedPrompt = `${prompt}, cartoon, vibrant, disney style`;
            else if (style === 'oil-painting') enhancedPrompt = `${prompt}, oil painting, textured, artistic`;
            else if (style === 'watercolor') enhancedPrompt = `${prompt}, watercolor, soft, flowing`;
            else if (style === 'sketch') enhancedPrompt = `${prompt}, pencil sketch, hand-drawn`;
            else if (style === 'pixel-art') enhancedPrompt = `${prompt}, pixel art, 16-bit, retro`;
            else if (style === 'horror') enhancedPrompt = `${prompt}, horror, dark, scary, eerie`;
            else if (style === 'sci-fi') enhancedPrompt = `${prompt}, sci-fi, futuristic, cyberpunk`;

            const prodiaUrl = 'https://api.prodia.com/v1/job';
            const prodiaKey = env.PRODIA_API_KEY;

            try {
                const jobRes = await fetch(prodiaUrl, {
                    method: 'POST',
                    headers: {
        ...
        // Redirect legacy image endpoints to unified /generate-image
        const STYLE_MAP = {
            '/ai/anime': 'anime',
            '/ai/realistic': 'realistic',
            '/ai/cartoon': 'cartoon',
            '/ai/oil-painting': 'oil-painting',
            '/ai/watercolor': 'watercolor',
            '/ai/sketch': 'sketch',
            '/ai/pixel-art': 'pixel-art',
            '/ai/horror': 'horror',
            '/ai/sci-fi': 'sci-fi',
        };

        for (const [legacyPath, style] of Object.entries(STYLE_MAP)) {
            if (path === legacyPath && (method === 'GET' || method === 'POST')) {
                const prompt = url.searchParams.get('prompt') || '';
                const newRequest = new Request(url.origin + '/generate-image', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
        ...
        if (path === '/tools/tts' && method === 'GET') {
            ...
            try {
                ...
                if (!apiRes.ok) {
                    return new Response(JSON.stringify({ error: 'TTS service unavailable' }), { status: 502, headers: corsHeaders });
                }
        ...
        // ----- PREXZYVILLA PROXY MAP (non-AI endpoints only) -----
