...
export default {
  async fetch(request, env, ctx) {
    ...
    const GIPHY_KEY = 'qnl7ssQChTdPjsKta2Ax2LMaGXz303tq';  // or move to env

    // All supported reactions (auto-generates /reactions/:name)
    const REACTIONS = [
    ...
    // Direct proxy map (GET or POST passthrough to PREXZY_BASE)
    const PROXY_MAP = {
    ...
    // ----------------------------- AUTH -----------------------------
    function isAuthenticated(request, url, env) {
    ...
    if (path === '/health') {
      ...
      return jsonResponse({ status: 'healthy', ...configStatus, timestamp: Date.now() });
    }
    ...
    if (path === '/auth/github/callback') {
      ...
      if (tokenData.error) return errorResponse(tokenData.error_description, 400);

      const userRes = await fetch('https://api.github.com/user', {
        headers: { 'User-Agent': 'Apex-CRYSNOVA', 'Authorization': `Bearer ${tokenData.access_token}` }
      });
      if (!userRes.ok) return errorResponse('Failed to fetch GitHub user', 500);
      const user = await userRes.json();
      const tempToken = await generateTempToken(env, user.id.toString());
      if (!tempToken) {
        return new Response(`<!DOCTYPE html><html><head><script>
          ...
          window.close();
        </script></head><body>Token expired. You may close this window.</body></html>`, {
    ...
    if (path === '/' && method === 'GET') {
      const html = await generateFrontendHTML(env);
      return new Response(html, { headers: { 'Content-Type': 'text/html;charset=UTF-8' } });
    }

    // ----------------------------- AUTHENTICATION GATE (API routes) -----------------------------
    const authHeader = request.headers.get('Authorization');
    const queryToken = url.searchParams.get('token');
    ...
    const isTempTokenValid = effectiveToken ? await validateTempToken(env, effectiveToken) : false;
    ...
    try {
      // ----- Helper for POST with external API forwarding -----
      async function forwardPost(targetUrl, reqBody, contentType) {
      ...
      if (path === '/tools/tojavascript' && method === 'GET') {
        ...
        const targetUrl = `${PREXZY_BASE}/tools/tojavascript?code=${encodeURIComponent(code)}&from=${encodeURIComponent(from)}`;
        const apiRes = await fetch(targetUrl);
        const data = await apiRes.json();
        return jsonResponse(data);
      }
      // Alias for frontend's /tools/tojs
      if (path === '/tools/tojs' && method === 'GET') {
        const code = url.searchParams.get('code') || '';
        const from = url.searchParams.get('from') || 'auto';
        if (!code) return errorResponse('Missing code');
        const targetUrl = `${PREXZY_BASE}/tools/tojavascript?code=${encodeURIComponent(code)}&from=${encodeURIComponent(from)}`;
      ...
      if (path === '/tools/tts' && method === 'GET') {
        ...
        if (!text) return errorResponse('Missing text parameter');
        ...
        const apiRes = await fetch(ttsUrl, { headers: { 'User-Agent': 'Mozilla/5.0' } });
        if (!apiRes.ok) return errorResponse('TTS service unavailable', 502);
      ...
      if (path === '/generate-image' && method === 'POST') {
        ...
        let baseUrl;
        if (category === 'horror') baseUrl = `${PREXZY_BASE}/ai/horror`;
        else if (category === 'sci-fi') baseUrl = `${PREXZY_BASE}/ai/sci-fi`;
        else if (category === 'pixel-art') baseUrl = `${PREXZY_BASE}/ai/pixel-art`;
        else baseUrl = `${PREXZY_BASE}/ai/realistic`;
      ...
      if (path === '/changebg' && method === 'POST') {
        ...
        if (!imageFile) return errorResponse('Missing image');
        ...
        if (!apiRes.ok) return errorResponse(`Upstream error: ${apiRes.status}`, apiRes.status);
        const imageBuffer = await apiRes.arrayBuffer();
        return new Response(imageBuffer, { headers: { ...CORS_HEADERS, 'Content-Type': 'image/jpeg' } });
      }
      ...
      if (path === '/remini' && method === 'POST') {
        ...
        if (!apiRes.ok) {
          ...
          return errorResponse(`Enhancement failed: ${apiRes.status} – ${err}`, apiRes.status);
        }
        const imageBuffer = await apiRes.arrayBuffer();
        return new Response(imageBuffer, { headers: { ...CORS_HEADERS, 'Content-Type': 'image/jpeg' } });
      }
      ...
      if (path === '/rembg' && method === 'POST') {
        ...
        if (!imageFile) return errorResponse('Missing image_file');
        ...
        if (!apiRes.ok) {
          const err = await apiRes.text();
          return errorResponse(`remove.bg error: ${apiRes.status} – ${err}`, apiRes.status);
        }
        const imageBuffer = await apiRes.arrayBuffer();
        return new Response(imageBuffer, { headers: { ...CORS_HEADERS, 'Content-Type': 'image/png' } });
      }
      ...
      if (path === '/deepseek' && method === 'POST') {
        ...
        const apiUrl = `${PREXZY_BASE}/ai/deepseekchat?prompt=${encodeURIComponent(trainingPrompt)}`;
        const apiRes = await fetch(apiUrl);
        const data = await apiRes.json();
        let reply = data?.result || data?.response || (typeof data === 'string' ? data : JSON.stringify(data));
        return jsonResponse({ success: true, message: { content: reply } });
      }

      if (path === '/chat' && method === 'POST') {
        const { prompt } = await request.json();
        if (!prompt) return errorResponse('Missing prompt');
        const apiUrl = `${PREXZY_BASE}/ai/chateverywhere?text=${encodeURIComponent(prompt)}`;
        const apiRes = await fetch(apiUrl);
        const data = await apiRes.json();
        let reply = data?.message || data?.reply || data?.response || (typeof data === 'string' ? data : JSON.stringify(data));
        return jsonResponse({ response: reply });
      }

      // Special nanobanana handlers
      if (path === '/ai/nanobanana' && method === 'GET') {
        ...
        return new Response(res.body, { headers: { ...CORS_HEADERS, 'Content-Type': res.headers.get('Content-Type') } });
      }
      ...
      // Reaction endpoints (auto-generated)
      if (path.startsWith('/reactions/') && method === 'GET') {
        const reaction = path.split('/')[2];
        if (REACTIONS.includes(reaction)) {
          const gifUrl = await fetchGif(`${reaction} anime`);
          if (!gifUrl) return errorResponse('No GIF found', 404);
          return jsonResponse({ url: gifUrl });
        }
      }

      // Proxy all remaining endpoints from PROXY_MAP
      if (PROXY_MAP[path] && (method === 'GET' || method === 'POST')) {
        const targetUrl = `${PREXZY_BASE}${PROXY_MAP[path]}?${url.searchParams.toString()}`;
        const init = { method };
        if (method === 'POST') {
...
// ----------------------------- FRONTEND HTML (separate function for readability) -----------------------------
async function generateFrontendHTML(env) {
